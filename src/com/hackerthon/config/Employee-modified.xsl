        ...
        <xsl:element name="Employee">
            ...
            <xsl:template match="employee_name">
                <xsl:element name="EmployeeFullName">
                    <xsl:value-of
                            select="concat(employee_name/first_name/text(),' ',employee_name/last_name/text())"/>
                </xsl:element>
            </xsl:template>
            <xsl:template match="address">
                <xsl:element name="EmployeeFullAddress">
                    <xsl:value-of
                            select="concat(address/no/text(),',',address/address_line1/text(),',',address/address_line2/text())"/>
                </xsl:element>
            </xsl:template>
